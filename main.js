...
//     return format + formattedValue; // Apply user-defined number format
// };
...
function calculateKpiValues(chartModel) {
    ...
    const measures = measureColumns.slice(1).map(col => {
        ...
        const change = value !== 0 ? ((mainKpiValue - value) / Math.abs(value)) * 100 : 0;
        return {
            label: col.name,
            value,
            change
...
async function render(ctx, format) {
    console.log("Inside render function. Number Format: " + format);
    const chartModel = await ctx.getChartModel();
    const kpiValues = calculateKpiValues(chartModel);
    updateKpiContainer(kpiValues.measures, kpiValues.mainKpiValue, format);
}

const renderChart = async (ctx, format) => {
    try {

        console.log("Inside renderChart. Number Format: " + format);
        ctx.emitEvent(ChartToTSEvent.RenderStart);
        await render(ctx, format);
    } catch (e) {
...
(async () => {
    const ctx = await getChartContext({
        ...
        renderChart: (ctx) => renderChart(ctx, userNumberFormat),
        ...
        onPropChange: (propKey, propValue) => {
            ...
            if (propKey === 'numberFormat') {
                ...
                renderChart(ctx, userNumberFormat); // Re-render the chart with the new format
            }
        },
    });

    renderChart(ctx, userNumberFormat);
})();
